...
namespace ScheduleView.Wpf.Controls
{
    ...
    {
        private static double dpiScale = 1.0f;
        ...
        private static bool isDpiScaleSet = false;

        private static void EnsureDpiScale()
        {
            if(isDpiScaleSet == false)
            {
        ...
        public static double RoundLayoutValue(double value)
        {
            EnsureDpiScale();
        ...
        /// <summary>
        /// If layout rounding is in use, rounds the size and offset of a rect by preserving original Width and Height
        /// </summary>
        ...
        /// the rounded value if it is unacceptable for layout, e.g. Infinity or NaN. It's a helper associated with
        /// UseLayoutRounding  property and should not be used as a general rounding utility.</remarks>
        private static Rect RoundLayoutRect(Rect rect, double dpiScaleX, double dpiScaleY)
        {
            var roundedX = RoundLayoutValue(rect.X);
            var roundedY = RoundLayoutValue(rect.Y);
            return new Rect(roundedX,
                roundedY,
                Math.Max(0, rect.Width - (roundedX - rect.X)),
                Math.Max(0, rect.Height - (roundedY - rect.Y)));
        }

        public static Rect RoundLayoutRect(Rect rect)
        {
            return RoundLayoutRect(rect, DpiScale, DpiScale);
        }

        public static Rect RoundLayoutRect2(Rect rect)
        {
            var rx = RoundLayoutValue(rect.X);
            var ry = RoundLayoutValue(rect.Y);
            var rwidth = Math.Max(0, RoundLayoutValue(rect.X + rect.Width) - rx);
            var rheight = Math.Max(0, RoundLayoutValue(rect.Y + rect.Height) - ry);

            return new Rect(rx, ry, rwidth, rheight);
        }

        public static Rect RoundLayoutRect3(Rect rect)
        {
            return new Rect(RoundLayoutValue(rect.X), RoundLayoutValue(rect.Y), RoundLayoutValue(rect.Width), RoundLayoutValue(rect.Height));
        }

        /// <summary>
        /// Source https://referencesource.microsoft.com/#WindowsBase/Shared/MS/Internal/DoubleUtil2.cs
        /// </summary>
        private static class DoubleUtil
        {
            ...
            // so please make sure to use DoubleUtil2.IsNaN() in performance sensitive code.
